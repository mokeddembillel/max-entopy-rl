...
class Debugger():
    ...
    def collect_data(self, o, a, o2, r, d, log_p, itr, ep_len):
    ...
    #         p = ax.scatter(x, y, z, c=q_values, cmap=plt.cm.viridis, alpha=1)
    #         # import pdb; pdb.set_trace()
    #         for j in range(particles.shape[2]):
    #             # print('########## ', str(i), particles[i, :, j, :])
    #             x_p, y_p, z_p = particles[i, :, j, 0], particles[i, :, j, 1], particles[i, :, j, 2]
    #             ax.plot(x_p, y_p, z_p, c='red')
    #         ax.set_xlabel('$a_1$')
    #         ax.set_ylabel('$a_2$')
    #         ax.set_zlabel('$a_3$')
    #         ax.set_xlim((-1, 1))
    ...
    def create_slices_plot(self, itr, fig_path, fig_size=(14, 7)):
        ...
        for k in range(min(500, len(obs))-1):
            ...
            if self.ac.pi.actor == 'svgd_nonparam':
            ...
            for i in range(len(_act_lst)):
                ...
                actions, _ = self.ac(o, deterministic=self.ac.pi.test_deterministic, with_logprob=False, all_particles=True)
                actions = actions.cpu().detach().numpy().squeeze()
                # if self.ac.pi.actor == 'svgd_nonparam':
                # print(actions)
                # else:
